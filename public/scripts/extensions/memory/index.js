...
function loadSettings() {
    ...
    $('#memory_override_response_length_value').val(extension_settings.memory.overrideResponseLength);
        ...
        switch (source) {
            ...
            case summary_sources.custom: {
                try {
                    ...
                    return await sendMemoryCustomApiRequest(messages);
                } catch (e) {
                    toastr.error(e.message, 'Custom API Error');
                    return '';
                }
            }
            default:
...
/**
 * Build a request body for the shared chat-completions backend (Custom API source).
 * @param {object[]} messages OpenAI-style messages array
 * @returns {object} Request body for /api/backends/chat-completions/generate
 */
function buildMemoryCustomApiRequestBody(messages) {
    const customKey = extension_settings.memory.custom_key || '';
    const body = {
        chat_completion_source: 'custom',
        custom_url: (extension_settings.memory.custom_url || '').replace(/\/+$/, ''),
    ...
    if (Number(extension_settings.memory.custom_max_tokens) > 0) {
        body.max_tokens = Number(extension_settings.memory.custom_max_tokens);
    }
...
/**
 * Send a summary request through the shared chat-completions backend and return the summary text.
 * @param {object[]} messages OpenAI-style messages array
 * @returns {Promise<string>} Summary text
 */
async function sendMemoryCustomApiRequest(messages) {
    const response = await fetch('/api/backends/chat-completions/generate', {
        method: 'POST',
        headers: getRequestHeaders(),
        body: JSON.stringify(buildMemoryCustomApiRequestBody(messages)),
    });
    ...
    if (!summary) {
        throw new Error('No summary generated from Custom API');
    }
...
/**
 * Get the currently activated World Info / Author's Note text for the summary context.
 * @param {object} context ST context
 * @returns {Promise<string>} World info string or empty string
 */
async function getWorldInfoText(context) {
    try {
        const chatForWI = context.chat
            .filter(m => !m.is_system && m.mes)
            .map(m => m.mes)
            .reverse();
        const result = await getWorldInfoPrompt(chatForWI, context.maxContext || 4096, true);
        return result?.worldInfoString || '';
    } catch (e) {
        console.warn('[Memory] Failed to get world info text:', e);
        return '';
    }
}

/**
 * Build the system prompt for summarization, including World Info / Author's Note.
 * @param {string} basePrompt User-configured summary prompt
...
async function summarizeChatCustom(context, force = false) {
    const basePrompt = await getSummaryPromptForNow(context, force);
    ...
    while (true) {
        ...
        // Avoid infinite loops if token budget prevents further progress.
        if (lastUsedIndex === previousLastUsedIndex) {
            console.warn('[Memory Custom] No progress made in cumulative summarization. Stopping.');
            break;
        }
...
async function summarizeChatMain(context, force) {
    ...
    while (true) {
        ...
        try {
            ...
            if (lastUsedIndex === previousLastUsedIndex) {
                console.warn('[Memory Main] No progress made in cumulative summarization. Stopping.');
                break;
            }
            previousLastUsedIndex = lastUsedIndex;
 ...
 * @param {object} context ST context
 * @param {string} systemPrompt Summarization system prompt (may include World Info)
 * @param {number|null} explicitStartIndex Index to start summarizing from
 * @param {number|null} targetEndIndex Maximum index to include (for cumulative chunking)
 * @param {string} previousSummariesText Text of previous summary stages to append to the user prompt
 * @returns {Promise<{rawPrompt: string, lastUsedIndex: number, messageCount: number}>} Raw summarization prompt
 */
async function getRawSummaryPrompt(context, systemPrompt, explicitStartIndex = null, targetEndIndex = null, previousSummariesText = '') {
    /**
     * Get the memory string from the chat buffer.
     * @param {boolean} includeSystem Include prompt and previous summaries into the memory string
    ...
    function getMemoryString(includeSystem) {
        ...
        const previousSummariesTrimmed = previousSummariesText.trim();

        if (includeSystem) {
            stringBuilder.push(systemPrompt);
        }

        stringBuilder.push(bufferString);

        if (previousSummariesTrimmed) {
            stringBuilder.push(previousSummariesTrimmed);
        }
    ...
    for (let index = startIndex; index < chat.length; index++) {
        ...
        // Respect the cumulative target end index if provided.
        if (targetEndIndex !== null && index > targetEndIndex) {
            break;
        }
    ...
    return { rawPrompt, lastUsedIndex, messageCount: chatBuffer.length };
}
...
function setupListeners() {
    ...
    $('#memory_prompt_words_force, #memory_prompt_words_force_value').off('input').on('input', onMemoryPromptWordsForceInput);
    ...
    function onMemoryCustomApiTempInput() {
        const value = $(this).val();
        extension_settings.memory.custom_temp = Number(value);
        $('#memory_custom_api_temp').val(extension_settings.memory.custom_temp);
        $('#memory_custom_api_temp_value').val(extension_settings.memory.custom_temp);
        saveSettingsDebounced();
    }

    function onMemoryCustomApiMaxTokensInput() {
        const value = $(this).val();
        extension_settings.memory.custom_max_tokens = Number(value);
        $('#memory_custom_api_max_tokens').val(extension_settings.memory.custom_max_tokens);
        $('#memory_custom_api_max_tokens_value').val(extension_settings.memory.custom_max_tokens);
        saveSettingsDebounced();
    }

    $('#memory_custom_api_temp, #memory_custom_api_temp_value').on('input', onMemoryCustomApiTempInput);
    $('#memory_custom_api_max_tokens, #memory_custom_api_max_tokens_value').on('input', onMemoryCustomApiMaxTokensInput);
    ...
    $('#memory_custom_api_fetch_models').on('click', async function() {
        try {
            ...
            if (!res.ok) {
                ...
                throw new Error(errorData.error?.message || errorData.error || res.statusText);
            }
