...
- Single-page app using vanilla JS + jQuery. No React/Vue.
...
3. Dynamic extension prompts (Memory, Author's Note, Vector Storage, depth-positioned WI) are injected at configured depths via `setExtensionPrompt()`.
...
- Dynamic components (World Info, Author's Note, Auto-Summary / Memory, Vector Storage) MUST be injected at `IN_CHAT` with `@Depth 0` or `@Depth 1`.
...
- World Info entries default to `atDepth` with `depth = 0`; Memory, Author's Note, and Vector Storage also default to `IN_CHAT @ Depth 0`.
