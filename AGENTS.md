...
- **New-chat bootstrap:** If a manual or automatic update finds no persisted active state tables, the extension builds a fresh canonical seven-table template in memory while preserving unrelated custom sheets. Singleton tables start with a blank `row_id=1`; multi-row tables start with headers only. The model initializes it from the configured chat history and optional Memory Summary. A first initialization is saved only when the validated edit makes at least one material data change; empty or same-value edits leave no checkpoint or success marker so the next request can retry. Partial snapshots keep every existing table unchanged, add only missing canonical tables, and receive a repair-specific prompt.
- Writes back: manual edits checkpoint to the latest non-user message; API updates atomically write both the checkpoint and success marker to the exact target Assistant message in one `saveChat()`. Failed saves restore the prior in-memory tag and marker.
- Parses `<tableEdit>` blocks from its dedicated update API (structured `updateRow/insertRow/deleteRow` commands, not SQL) and can update manually or at the configured AI-response interval. Active-table snapshots self-heal missing, invalid, or duplicate `row_id` values in memory while preserving valid IDs.
- **Atomic table edits:** Operations retain model order and are validated as one batch. Unknown tables/rows/columns, malformed residue, structured cell values, model-supplied `row_id`, or forbidden operations reject the whole batch without saving or marking success. `global_state`, `protagonist_info`, and `options` are update-only; `important_characters` cannot be deleted.
- **Write constraints:** Model inserts enforce non-empty identity fields and normalized unique business keys for character names, skill names, item names, and quest names. Renaming a row to an existing key is rejected, inventory quantity must stay a positive integer, and character presence must be `是` or `否`. Safe DDL defaults are applied for new inventory quantity (`1`) and character presence (`否`). Existing legacy duplicates do not block unrelated field updates, but no new duplicate can be created.
- **State vs. Memory:** Memory owns detailed chronology; state tables keep current facts and compact long-term conclusions. Important-character history is a rewritten relationship summary (soft target 80–160 Chinese characters), protagonist history is a compact growth/identity summary (soft target 200–300), and unrelated rows are not rewritten merely for cleanup. `key_items` stays setting-neutral and contains only currently held possessions with continuing narrative relevance. These are prompt-only soft limits; the executor never truncates cell values.
...
- **Async safety:** A state response is written only when the original chat and exact target Assistant message still exist unchanged and remain the latest Assistant reply; switched, edited, deleted, regenerated, or superseded targets are discarded.
- UI: a large freely draggable two-column popup with a sidebar and continuous editable record cards for all active tables + Memory. It opens from the drawer body, the drawer-title window button, or the lower-left magic-wand menu; the wand keeps Memory immediately before Protagonist State regardless of asynchronous extension load order. Its settings drawer is fixed in the right extension-settings column immediately below Memory and uses a dedicated Position/Depth/Role grid so global Flex styles cannot collapse the selects. The popup uses transform-free centering, resets to the center whenever opened, and only switches to explicit coordinates after the header actually moves. Dragging remains unconstrained and does not clamp against the viewport. A collapsible bottom bar (`#protagonist_state_bottom_bar`) shows selected-table summaries.
- Formats seven active tables (`global_state`, `protagonist_info`, `important_characters`, `protagonist_skills`, `inventory`, `quests_events`, `options`). Deprecated `sheet_3NoMc1wI` chronicle data is discarded during snapshot reconstruction and is never displayed, injected, updated, or checkpoint-saved.
...
- Legacy important-character snapshots may have the seven-column layout `row_id/name/gender_age/appearance/key_items/is_absent/past_experience`, while the current schema also includes `brief_intro`. Snapshot reads detect that exact legacy Chinese header, insert an empty `brief_intro`, realign the later fields, and merge any accidentally duplicated trailing `past_experience` without modifying unrelated/custom layouts.
...
- New default templates no longer create a `chronicle` sheet or instruct the table-filling prompt to write one. Deprecated chronicle sheets are excluded from active sorting, prompt assembly, visualization, reconstruction, and future checkpoint saves.
