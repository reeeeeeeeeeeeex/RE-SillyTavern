...
- **New-chat bootstrap:** If a manual or automatic update finds no persisted state snapshot, the extension builds a fresh canonical seven-table template in memory. Singleton tables start with a blank `row_id=1`; multi-row tables start with headers only. The model initializes it from the configured chat history and optional Memory Summary, and the template is saved only after a valid API response, so failures leave no empty checkpoint and no state can leak from another chat.
