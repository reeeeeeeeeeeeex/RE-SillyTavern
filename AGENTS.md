...
- Parses `<tableEdit>` blocks from its dedicated update API (structured `updateRow/insertRow/deleteRow` commands, not SQL) and can update manually or at the configured AI-response interval. Active-table snapshots self-heal missing, invalid, or duplicate `row_id` values in memory while preserving valid IDs and legacy chronicle data.
